...
Will this agent prefer this lottery to a payoff of $(200 × 0.6 + 100 × 0.4) = 160$ without any risk?
...
::: {.column width="50%"}
![](figures/Riskpremium1.png){.absolute top=150 right=0 width=50%}
:::
:::
...
::: {.column width="50%"}
![](figures/Riskpremium3.png){.absolute top=150 right=0 width=50%}
:::
:::
...
:::
::: {.column width="50%"}
![](figures/Riskpremium2.png){.absolute top=150 right=0 width=50%}
:::
:::
...
Lottery A: Get $10M for sure.

Lottery B: Get $0 with prob. 1%, $10M with prob. 89%, and $50M with prob. 10%.
...
Lottery A: Get $0 with prob. 89%, get $10M with 11% prob.

Lottery B: Get $0 with prob. 90%, get $50M with 10% prob.
...
$U(10, 000, 000) > 0.89U(10, 000, 000) + 0.1U(50, 000, 000)$ if $U(0) = 0$

Organize this expression, we get:

$0.11U(10, 000, 000) > 0.1U(50, 000, 000)$
...
![](figures/kahneman_tversky.jpg){width=70%}
...
![](figures/prospect2.jpg){width=80%}
...
People don't treat probabilities linearly. Instead, they:

- Overweight small probabilities (rare events loom larger than they should)
- Overweight certainty (placing extreme value on eliminating all risk)
- Underweight moderate probabilities (middle-range probabilities as less important)

**Example 1: The Certainty Effect**
A patient deciding on an expensive medication values probability changes very differently:

- 0% → 1% chance of recovery: Willing to pay a LOT (overweighting the small probability)
- 50% → 51% chance of recovery: Willing to pay relatively little
- 99% → 100% chance of recovery: Willing to pay a LOT (overweighting certainty)

Under *expected utility theory*, all three scenarios represent the same 1 percentage point improvement and should be valued equally. They're not.

---

## Prospect Theory: Probability Weighting

**Example 2: Why People Play the Lottery**

- In Canada, the probability of winning a major lottery jackpot is 1 in 13,983,816. You are far more likely to:
  - Die in a terrorist attack while traveling (1 in 650,000)
  - Be killed by lightning (1 in 56,439)

Yet millions of people regularly buy lottery tickets, demonstrating massive overweighting of tiny probabilities. The 0.000007% chance of winning gets treated psychologically as if it's far more likely.

**Key Insight:** The psychological weight people assign to probabilities follows an S-shaped curve, not a straight line. This explains seemingly irrational behaviors around insurance, gambling, and risk-taking.
...
![](figures/prob_weight.png){width=70%}
...
- Say that you are down 0-1, and you pull the goalie with 6 min left and lose 0-5, does the coach looks stupid? Yes, to those that don't understand probabilities. 
  - A coach prefers to lose 0-1 or 0-2 than 0-5. The coach looks more credible, more likely to keep is job.
...
- This is puzzling. 
  - Stocks that have recently done well continue to outperform, on average, while those that have done poorly continue to underperform. 
  - As such, investors should concentrate their selling among stocks with poor past performance—but they do the opposite.
...
- If a stock performs poorly, this brings its owner in to the loss stock region of the value function, where, because of the convexity, the owner becomes risk-seeking 
  - $\rightarrow$ willing to hold on the stock to break even.
- If a stock performs great, this brings its owner in to the gain stock region of the value function, where, because of the concavity, the owner becomes risk-averse. 
  - $\rightarrow$ sell and realize gains.
...
Terry Odean, in this [paper](https://onlinelibrary.wiley.com/doi/full/10.1111/0022-1082.00072?casa_token=FJrSMnkEL9IAAAAA%3AsUz_4PAYrh2C1CMxJJUaXwwd9kapCjc4tHt0Q-8wTG0bqIpyOnzMHrt_A6fc0NSLOeAcTdP21t-Qe6zc), examines the disposition effect, the tendency of investors to hold losing investments too long and sell winning investments too soon, by analyzing trading records for 10,000 accounts at a large discount brokerage house.
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/Y2ECnsx1cvs?si=5B8SgxhCDTBGj6Us" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
![](figures/frazzini1.PNG){width=60% fig-align="center"}
...
![](figures/frazzini2.PNG){width=60% fig-align="center"}
...
![](figures/frazzini3.PNG){width=70% fig-align="center"}
...
![](figures/frazzini4.PNG){width=70% fig-align="center"}
...
Frazzini confirms his hypothesis empirically. 

- He first calculates capital gains (losses) overhang as the percentage deviation of the aggregate cost basis from the current price:
- $g_t = \frac{p_t − rp_t}{p_t}$, where $p_t$ is the current price and $rp_t$ is the reference price of the aggregate investor calculated from mutual fund holdings. Think about it as the "mental" gain (or loss).
...
![](figures/arrows.png){width=50% fig-align="center"}
